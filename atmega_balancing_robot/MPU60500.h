...
void get_MPU6050()
{
  ...
  g_acc[0] = ((double)g_acc_raw[0]) / 16384.0 - 0.04f;
  g_acc[1] = ((double)g_acc_raw[1]) / 16384.0 - 0.03f;
  g_acc[2] = ((double)g_acc_raw[2]) / 16384.0 - 0.38f;
  g_gyro[0] = ((double)g_gyro_raw[0]) / 131.0 + 5.392;
  g_gyro[1] = ((double)g_gyro_raw[1]) / 131.0 + 1.572f;
  g_gyro[2] = ((double)g_gyro_raw[2]) / 131.0 - 0.417f;
}
